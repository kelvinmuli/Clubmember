...
document.addEventListener("DOMContentLoaded", function () {
    ...
    let started = false; // Prevents multiple triggers
    ...
    let observer = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                startCounters();
                observer.disconnect(); // Stops observing after activation
            }
        });
    }, { threshold: 0.5 });

    observer.observe(document.querySelector(".fun-facts-area"));
});
