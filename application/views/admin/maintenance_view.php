				...
				<div class="col-auto ms-auto d-print-none">
					<div class="btn-list">
						...
						<button class="btn btn-success d-none d-sm-inline-block btn-pill" onclick="addModuleSetupModal('<?= $module_type_id ?>')">
							<svg xmlns="http://www.w3.org/200/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
								<path stroke="none" d="M0 0h24v24H0z" fill="none" />
								<line x1="12" y1="5" x2="12" y2="19" />
								<line x1="5" y1="12" x2="19" y2="12" />
							</svg> Add <?= get_table('m_module_type', 'module_type_id', $module_type_id, 'name') ?> Setup
						</button>
				...
				<div class="card-body">
										...
										<div>
											...
											<div class="card-table table-responsive py-2">
												<table id="maintenance-<?= $maintenance ?>-datatable" class="table card-table table-vcenter text-wrap datatable">
													<thead>
														<tr>
															<?php if (isset($maintenanceFieldData[$maintenance])): foreach ($maintenanceFieldData[$maintenance] as $maintenanceField): $editSysControl = !get_sys_control(get_table('maintenance', 'name', $maintenance, 'maintenance_id'), 'edit', 1);
																	$removeSysControl = !get_sys_control(get_table('maintenance', 'name', $maintenance, 'maintenance_id'), 'remove', 1); ?>
															...
															if ($editUserRight && $editSysControl || $removeUserRight && $removeSysControl): ?>
																<th>Action</th>
															<?php endif; ?>
														</tr>
													</thead>
													<?php $pos = 0;
													if (isset($maintenanceFieldValueData[$maintenance])): foreach ($maintenanceFieldValueData[$maintenance] as $maintenanceFieldValue): ?>
															<tr>
																<?php if (isset($maintenanceFieldData[$maintenance])): foreach ($maintenanceFieldData[$maintenance] as $maintenanceField): $column = $maintenanceFieldData[$maintenance][1]->name; ?>
																		<td <?php $name = $maintenanceField->name;
																			$combineColumnToTable = get_combine_column_to_table($name); ?>>
																			...
																			<?php } elseif (in_array($name, array('sold_color_id')) && 'm_color' != $maintenance) { ?>
																				...
																				<?php } elseif (in_array($name, explode(',', get_table('m_column_define', 'column_define_id', '1753528719052', 'multi_column'))) && 'm_choice' != $maintenance) {
																				foreach ($maintenanceFinalData['choiceData'] as $key => $value) { ?>
																			...
																			<?php } else {
																				echo ($maintenanceFieldValue->$name);
																			} ?>
																		</td>
																	<?php endforeach;
																endif;
																$editSysControl = !get_sys_control(get_table('maintenance', 'name', $maintenance, 'maintenance_id'), 'edit', 1);
																$removeSysControl = !get_sys_control(get_table('maintenance', 'name', $maintenance, 'maintenance_id'), 'remove', 1);
																if ($editUserRight && $editSysControl || $removeUserRight && $removeSysControl): ?>
																	<td>
																		...
																		<?php endif;
																		if ($removeUserRight && $removeSysControl): ?>
					...
					<?php else: ?>
						<div class="accordion accordion-tabs" id="accordion-example">
							<?php $count = 1;
							if (isset($sustenanceData)): foreach ($sustenanceData as $key => $maintenance):  $tableName = get_broken_name($maintenance, '_', 1); ?>
