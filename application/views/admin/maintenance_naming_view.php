<div class="page-wrapper">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body border-bottom py-3">
                            <div class="table-responsive">
                                <table id="maintenance-naming-datatable" class="table card-table table-vcenter text-nowrap datatable" style="width: 100%;">
                                    <thead>
                                        <tr>
                                            <th>#</th>		
                                    ...
                                    <tbody>											
                                        <?php $mn = 0; if (isset($maintenanceNamingData)): foreach($maintenanceNamingData as $maintenance): ?>
                                            <tr>
                                                <td><?=++$mn?>.</td>
                                                <td><?=get_broken_name($maintenance->maintenance, '_', 1)?></td>
                                                <td><?=$maintenance->name?></td>
                                                <td><span class="badge badge-outline text-<?=$maintenance->active == 1 ? 'success' : 'danger'?>"><?=get_table('m_active', 'num', $maintenance->active, 'name')?></span></td>
													...
													<span class="dropdown">
														...
														<div class="dropdown-menu dropdown-menu-end">
                                                            <?php if ($editUserRight): ?>
																<a class="dropdown-item" onclick="editMaintenanceNamingModal('<?=$maintenance->maintenance_naming_id?>')">Edit <?=get_broken_name($maintenance->maintenance, '_', 1)?></a>
															<?php endif; ?>
														</div>
													</span>
												</td>
	...
	<script>
		$(document).ready(function() {
			loadDatatable('maintenance-naming-datatable');
		});
		...
		function editMaintenanceNamingModal(maintenance_naming_id) {	
			$.ajax({
				url: base_url + "edit-maintenance-naming-modal/" + maintenance_naming_id,
				success: function(response) {
					document.getElementById('modal-view-edit-print-maintenance').innerHTML = response;
					$('#modal-view-edit-print-maintenance').modal('show');
				}
