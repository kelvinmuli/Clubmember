<?php
$pageTitle = isset($moduleMenu) && !empty($moduleMenu->name) ? $moduleMenu->name : 'AGM Minutes';
$modulePath = isset($moduleMenu) && !empty($moduleMenu->path) ? $moduleMenu->path : 'agm-minutes';
?>
				...
				<div class="col-auto ms-auto d-print-none">
					<div class="btn-list">
						<?php if ($inputUserRight): ?>
							<a class="btn btn-primary btn-pill" onclick="addAGMMinutesModal();" href="javascript:void(0);">
								<!-- Download SVG icon from http://tabler.io/icons/icon/add -->
					...
					<div class="card">
						...
						<div class="card-body border-bottom py-3">
							<div class="table-responsive">
								<table id="agm-minutes-datatable" class="table table-vcenter text-wrap" style="width: 100%;">
									<thead>
										<tr>
											<th class="w-1">#</th>
											<th>Title</th>
									...
									<tbody>
										<?php $am = 0; if (isset($agmMinutesData)): foreach ($agmMinutesData as $minute): ?>
											<tr>
												<td><?=++$am?></td>
												<td><?=$minute->name?></td>
												<td><div class="only-so-big text-muted"><?=$minute->description?></div></td>
												<td><?=!empty($minute->date_at) ? date('d M Y', strtotime($minute->date_at)) : 'N/A'?></td>
												<td>
													<?php if (check_file_exists($minute->doc_url)): ?>
														<a href="<?=base_url($minute->doc_url)?>" class="btn btn-outline-primary btn-sm btn-pill" target="_blank" rel="noopener">
															<svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round"><path stroke="none" d="M0 0h24v24H0z" fill="none"/><path d="M14 3v4a1 1 0 0 0 1 1h4" /><path d="M5 9v-2a2 2 0 0 1 2 -2h7l5 5v9a2 2 0 0 1 -2 2h-10a2 2 0 0 1 -2 -2v-11" /><path d="M9 17l6 -6" /><path d="M15 17h-6v-6" /></svg> View
														</a>
													<?php else: ?>
														...
														<span class="dropdown">
															...
															<div class="dropdown-menu dropdown-menu-end">
																...
																<?php endif; if ($removeUserRight): ?>
																	<a href="javascript:void(0);" class="dropdown-item" onclick="removeAgmMinutesModal('<?=$minute->agm_minutes_id?>')">
